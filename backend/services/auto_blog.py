"""
Auto-blog orchestrator.

Generates a SEO-friendly + GEO-friendly (Generative-Engine + geographic) blog
post on a fixed cadence using:
  - Claude Sonnet 4.5 for body, title, FAQ, JSON-LD, geo meta
  - Gemini Nano Banana for hero (16:9) + 2 inline images
...
Cycle settings live in `admin_settings.auto_blog`:
  ...
  - cadence_days (number, default 3.5 for twice/week)
...
def md_to_html(text: str) -> str:
    """Convert Claude's markdown body to HTML so the existing blog renderer
    ...
    if not text:
        return ""
    # Strip FAQ section — rendered separately via post.faqs
    parts = re.split(r"^##\s*Frequently Asked Questions\b.*$", text, maxsplit=1, flags=re.MULTILINE | re.IGNORECASE)
    body = parts[0]
...
def _next_due(last_run_iso: Optional[str], cadence_days: float) -> str:
    base = (
...
async def _pick_topic(db, cycle_count: int) -> dict:
    ...
    Skips topics already used in the last 90 days."""
    ...
    if not use_trend:
        # Linear walk through the bank, skipping recently-used topics
        ...
        # All recently used? Pick by index anyway
...
def _strip_json_fence(text: str) -> str:
    """Claude sometimes wraps in ```json ... ``` despite instruction. Defang."""
    t = text.strip()
    if t.startswith("```"):
        t = re.sub(r"^```(?:json)?\s*", "", t)
        t = re.sub(r"\s*```$", "", t)
...
async def _generate_text(topic: dict) -> dict:
    """Generate text via Claude Sonnet 4.5 with auto-retry on transient 5xx."""
    last_err = None
    for attempt in range(3):
        try:
            chat = LlmChat(
                api_key=EMERGENT_LLM_KEY,
                session_id=f"auto-blog-{uuid.uuid4().hex[:8]}",
                system_message=SYSTEM_PROMPT,
            ).with_model(*TEXT_MODEL)
            msg = UserMessage(text=_build_user_prompt(topic))
            ...
            logger.error(f"Auto-blog: invalid JSON on attempt {attempt+1}: {e}")
            last_err = e
        except Exception as e:
            msg_str = str(e)
            # Hard-fail on budget / auth errors — retrying just burns more time
            if any(s in msg_str for s in ("Budget has been exceeded", "Insufficient", "AuthenticationError", "InvalidApiKey")):
                raise RuntimeError(f"LLM budget/auth issue (won't retry): {msg_str}") from e
            # Retry only on transient gateway / rate-limit errors
            if any(s in msg_str for s in ("502", "503", "504", "BadGateway", "rate", "Timeout", "timeout")):
                logger.warning(f"Auto-blog: transient error attempt {attempt+1}: {msg_str[:120]}")
                last_err = e
                if attempt < 2:
...
async def _generate_image(prompt: str) -> Optional[bytes]:
    ...
    try:
        chat = LlmChat(
            api_key=EMERGENT_LLM_KEY,
            session_id=f"auto-blog-img-{uuid.uuid4().hex[:8]}",
            system_message="You are a photo-realistic image generator.",
        ).with_model(*IMAGE_MODEL).with_params(modalities=["image", "text"])
        _, images = await chat.send_message_multimodal_response(UserMessage(text=prompt))
        if not images:
            logger.warning(f"Auto-blog: no image returned for prompt: {prompt[:80]}")
            return None
        return base64.b64decode(images[0]["data"])
    except Exception as e:
        logger.error(f"Auto-blog image generation failed: {e}")
        return None
...
async def run_one_cycle(db, force: bool = False, admin_email: str = "system:auto") -> dict:
    """Run a single auto-blog cycle. Returns a result dict suitable for logging.

    Pass `force=True` to skip the next-due check (used by the admin "Run now"
    button)."""
    ...
    hero_bytes, in1_bytes, in2_bytes = await asyncio.gather(
        _generate_image(gen["hero_image_prompt"]),
        _generate_image(gen.get("inline_image_1_prompt") or ""),
        _generate_image(gen.get("inline_image_2_prompt") or ""),
    )
    ...
    # 4) JSON-LD article schema (BlogPosting + FAQPage)
    ...
    doc = {
        ...
        # Snake-case keys to match the existing router's queries
    ...
    # 5) Update settings — bump cycle, set last_run / next_due
    await update_settings(
        db,
        {
            "cycle_count": cycle_count + 1,
            "last_run_at": now.isoformat(),
            "next_due_at": _next_due(now.isoformat(), float(cfg["cadence_days"])),
        },
...
async def scheduler_loop(db):
    ...
    fires Claude+Gemini calls when actually due (twice/week by default)."""
    ...
    while True:
        try:
            ...
            if not cfg.get("next_due_at"):
                # First-time bootstrap: set next_due to NOW so we run immediately
                await update_settings(
                    db,
                    {
            ...
            try:
                due = datetime.fromisoformat(
                    cfg["next_due_at"].replace("Z", "+00:00")
                )
            except Exception:
                continue
