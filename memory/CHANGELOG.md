...
- ✅ **Env wired** in `backend/.env`: `PARTNER_SHARED_SECRET=<redacted>`, `AMARDEEP_API_BASE=https://aaroviah-retail.preview.emergentagent.com`, `PARTNER_MIN_ORDER_INR=499`.
...
## Iter102 — Mobile OTP registration + OTP login + External stock API keys + Supabase mirror — 2026-09-21
- **Mobile (Aaroviah) full registration**: new `mobile/app/register.tsx` — business/contact/email/+91 phone with inline SMS-OTP verify, GST number, GST-certificate upload (expo-document-picker ~12.0.2, multipart to /api/retailer-auth/register), password+confirm. Blocks submit until phone verified; auto-logs-in via `applyRetailerSession` on success. Route wired in `_layout.tsx` (+ auth gate treats `register` as an auth screen).
- **Mobile OTP login**: `mobile/app/login.tsx` now has a Password | OTP toggle. OTP mode calls `/phone/login-send-otp` (backend confirms a retailer exists for that number BEFORE sending; 404 otherwise) then `loginRetailerOtp` → `/phone/login-verify`. Signup link now opens the in-app `/register` (was web deep-link). session.ts gained `loginRetailerOtp` + `applyRetailerSession`.
- **Backend OTP login** (`routers/retailer_auth.py`): `POST /api/retailer-auth/phone/login-send-otp` (registered-number gate, suspended check, cooldown, DEV dev_code) and `POST /phone/login-verify` (verifies OTP, issues retailer session identical to password login). Helper `_find_retailer_by_phone` matches by phone-suffix regex, excludes deleted.
- **External stock API** (`routers/external_api.py`, mounted `/api/external/v1`): `GET /ping`, `GET /stock` (filters: category, active_only, pagination), `GET /stock/{id}` (SKU or parent product_id). Auth via `X-API-Key` header (or Bearer), scope `stock:read`. Reads MongoDB `b2b_products` (source of truth). Fields: id, product_id, name, category, net_weight, stock_pieces, stock_status, price_per_unit (mrp_per_unit), price_per_carton, is_active.
- **API key system** (`services/api_keys.py`): random `addk_` secret, only sha256 hash stored, shown once. Admin router `routers/admin/admin_api_keys.py` → `/api/admin/api-keys` (GET list, POST create, POST {id}/revoke, DELETE {id}). Web admin panel `frontend-next/app/admin/api-keys/page.js` + nav entry (KeyRound). Dark-mode contrast fixed per testing feedback.
- **Supabase consistency**: retailer `/register` now mirrors to Supabase (password stripped); admin status changes (`admin_retailer_requests._apply_status`) re-mirror the updated retailer. Blocklist extended: `phone_otps`, `phone_verifications`, `api_keys` never leave Mongo.
- **Tested (iteration_101.json)**: backend 13/13 pytest pass; web `/admin/api-keys` full Playwright E2E pass (create → one-time reveal → use as X-API-Key → revoke → 401 → delete). OTP login (unregistered 404, registered dev_code → verify → session), external auth (401 without key, 200 with), regression password login OK. Mobile RN verified via `tsc --noEmit` only (cannot browser-test). Twilio still DEV mode (no keys).

## Iter103 — Live stock-change webhooks + EAS Android build initiated — 2026-09-21
- **Stock webhooks**: new `services/stock_webhooks.py` fired from the single chokepoint `b2b_inventory.adjust_stock` (best-effort, non-blocking asyncio). Events: `stock.changed` (every move), `stock.low` (below pieces_per_carton × threshold), `stock.out` (zero). Highest-severity single event per webhook to avoid noise.
- **Delivery security**: each POST carries `X-Addrika-Event`, `X-Addrika-Delivery` (uuid), and `X-Addrika-Signature: sha256=<HMAC-SHA256(body, secret)>`. Signing secret (`whsec_…`) shown once. Deliveries logged to `stock_webhook_deliveries` (recent 20 in admin).
- **Admin API** `routers/admin/admin_stock_webhooks.py` → `/api/admin/stock-webhooks`: GET (list + available_events + recent_deliveries), POST create (returns one-time secret), POST {id}/toggle, POST {id}/test (sends sample `stock.test`), DELETE {id}.
- **Admin UI** `frontend-next/app/admin/stock-webhooks/page.js` + nav entry (Webhook icon) — register endpoint, event checkboxes, threshold in cartons, one-time secret reveal, per-row Test/Pause/Delete, recent-delivery feed. Dark-mode styled.
- **Tested**: admin create → one-time secret; `/test` delivered `stock.test` with signature verified valid; real `adjust_stock` path delivered `stock.low` (after 7 < threshold 12) and `stock.out` (after 0). Collections: `stock_webhooks`, `stock_webhook_deliveries`. ruff clean; imports OK.
...
## Iter105 — D2C Out-of-Stock UI fixed (grid + PDP) — 2026-06-26
- **Root cause of iteration_102 OOS failures**: the PDP/QuickView OOS code was already correct, but supervisor serves the web app with `next start` (PRODUCTION build) from `/app/frontend` (symlink → `/app/frontend-next`). The running build was stale, so no OOS UI appeared. Documented in `memory/ENV_NOTES.md`: every frontend change now needs `cd /app/frontend && yarn build && sudo supervisorctl restart frontend`.
- **Grid fix** (`components/FragranceGridServer.js`): ProductCard now derives `isOutOfStock` (all sizes tracked and every `stock <= 0`, and NOT comingSoon) and renders a rose "Out of Stock" pill with `data-testid="product-oos-badge-{id}"` instead of falling back to the category/"Coming Soon" tag.
- **Verified live**: `/products/kesar-chandan` → `size-50g-oos` + `size-200g-oos` labels, `out-of-stock-msg` copy, Notify Me button, and NO `add-to-cart-btn`. `/products` → 7 OOS badges. Mobile 390px checked (only pre-existing decorative overflow).
- **Deploy readiness**: `yarn build` clean (all routes generated) and `yarn ci` (lint + brand-audit) passes — brand-audit reports 0 hardcoded brand refs across 154 files. Lint gate is wired via `yarn ci` in `frontend/package.json`.

## Iter106 — 4 requested features + GSTIN becomes the retailer login ID — 2026-06-26
**Feature batch (tested: iteration_105.json, 24/24 backend + all frontend flows):**
- **Onboarding walkthrough**: new public page `/retailer/onboarding` + `components/AarovihaWalkthrough.js` — a 60-second, 6-chapter animated walkthrough (Browse → Build cart → Continue on web → Secure token → Auto login → Pay & track) with phone/desktop CSS mockups, auto-play, pause/resume, restart, clickable chapter chips and a timecode. No video file needed (user chose the animated option). Entry point: `wholesale-walkthrough-cta` on /wholesale. `app/retailer/layout.js` exempts the page from the retailer auth gate.
- **Low stock nudge**: `FragranceGridServer.js` ProductCard shows an amber pulsing "Only X left" pill (`product-low-stock-{id}`) when the lowest positive tracked size stock is ≤ 12; OOS pill (`product-oos-badge-{id}`) when every tracked size is 0; "Coming Soon" always wins over OOS.
- **Restock alerts (approval-gated, user's choice)**: `services/restock_alerts.py` — `record_restock_candidate` fires from `b2b_inventory.adjust_stock` ONLY on a 0 → positive transition and only when someone is waiting; dedupes on `status='pending'`. `routers/admin/admin_restock_alerts.py` → GET `/api/admin/restock-alerts`, POST `/{id}/approve` (emails the waitlist, stamps `notified_at`, 400 if already sent), POST `/{id}/dismiss`. Admin UI: "Restock Alerts Awaiting Approval" section on `/admin/notify-me`. Resend is LIVE — a real back-in-stock email was delivered during testing.
- **Notify-Me gate relaxed**: `POST /api/notify-me` now accepts out-of-stock live products (sum of linked `b2b_products.stock_pieces` == 0), not only Coming-Soon ones. Re-subscribing clears `notified_at`.
- **Newsletter**: `/blog` SmokeSignalSubscribe → `POST /api/subscribe` verified end-to-end (welcome email + admin subscriber listing).

**GSTIN = retailer username (tested: iteration_106.json, 15/15 backend + web UI):**
- `services/retailer_identity.py` — GSTIN normalize/validate, `ensure_gstin_usernames()` boot migration (username := GSTIN for every live account; live accounts with no valid GSTIN are soft-deleted with `gstin_migration_deactivated`; non-unique `gst_number` index — duplicates are blocked in the app layer so closed accounts can re-register), plus login lockout helpers (10 fails / 15 min, collection `retailer_login_attempts`).
- `retailer_auth.login` — accepts `{gstin, password}` (legacy `email`/`username` keys are treated as the GSTIN input), rejects anything containing `@` with 400 "Retailer logins use your 15-character GSTIN, not your email address.", validates the GSTIN pattern, looks up `gst_number`/`username` ONLY (never email), non-leaky 401 "Invalid GSTIN or password", returns `gst_number` + `username` in the payload. `RTL_TEST_B2B`'s legacy username `test_b2b_retailer` is allowlisted.
- `retailer_auth.register` — 409 "An account already exists for this GSTIN." and stores `username = gst`. `retailers.admin_create` forces `username = gst_number`.
...
## Iter108 — Security audit remediation (all 5 findings closed) + GSTIN notice sent — 2026-06-26
Verified: `/app/test_reports/iteration_108.json` — 23/23 security tests pass, all of SEC-001…SEC-005 confirmed closed, no regressions. Suite: `backend/tests/test_iter108_security_audit.py`.

- **SEC-001 (HIGH) unauthenticated store-pickup completion** — `routers/store_pickup.py`: `/verify-otp` now requires a retailer session (and the order must belong to that store) **or** an admin session. The hard-coded `ADMIN_MASTER_PASSWORD` override is **deleted** (field removed from the schema too); admins use `admin_override: true` with a real admin session instead. Added a brute-force guard on the 6-digit customer OTP (5 attempts → 15-min lock). Deleted the stale `tests/test_store_pickup_hybrid_verification.py` that asserted the old behaviour.
- **SEC-002 (HIGH) public retailer PII leak** — `/api/retailers/by-location` switched from an exclusion projection to the same explicit `PUBLIC_FIELDS` whitelist the map endpoint uses; email, phone, GSTIN, SPOC, admin notes and KYC state no longer reach anonymous callers.
- **SEC-003 (HIGH) admin PIN-recovery backdoor** — `admin/admin_auth.py`: the static `addrika_admin_override` string is gone; recovery accepts only the emailed OTP, compared with `secrets.compare_digest`, still capped at 5 attempts.
- **SEC-004 (MED) fail-closed secrets** — `services/auth_service.py` now requires `JWT_SECRET_KEY` and `ADMIN_DEFAULT_PIN` via `os.environ[...]` (boot fails if absent) and `verify_admin_pin` denies instead of comparing a plaintext default when no `admin_settings` row exists. Also removed PIN values from log lines.
- **SEC-005 (MED) external API blast radius + webhook SSRF** — `routers/external_api.py`: per-key rate limit (120 req/min, `api_key_rate_limits` with a TTL index, applied to `/ping` too) and optional `retailer_ids` scoping enforced on `/retailers`, `/orders`, `/orders/{id}`, `/orders/{id}/cancel` and order placement/preview (403 "not scoped to that retailer"); the free-text `q` search is now `re.escape`d (ReDoS). `services/stock_webhooks.py`: new `is_public_http_url()` SSRF guard rejects loopback/private/link-local/reserved/`.local`/metadata hosts and non-http schemes, enforced both at delivery time and in the admin create/update validators. Existing unrestricted keys keep working (documented: empty `retailer_ids` = unrestricted).
- **Dev OTP leak gated** — `phone_otp.dev_codes_exposed()` / `auth._dev_otp_exposed()`: a one-time code is echoed in API responses ONLY when `ALLOW_DEV_OTP=1`. Set to 1 in this preview env; **must stay unset in production**.
- **Password strength** — `retailer_password_reset.password_problems()` blocks <8 chars, common passwords, all-digit and repetitive passwords (422 with a readable reason); `/retailer/reset-password` shows a live strength meter (`reset-password-strength`) and refuses to submit a very weak password.
...
## Iter110 — Waitlist "pseudo-onboard" fix: admin notification email outcome is now always recorded — 2026-06-27
Verified: `/app/test_reports/iteration_110.json` — 9/9 executed tests pass (3 principled skips, covered in-process), 0 issues.

- **Bug**: `/retailer/login` waitlist signup told the retailer they'd joined the waitlist but NO email reached contact.us@centraders.com. Root cause: `create_waitlist_signup` called `send_email` inside a bare try/except — failures (missing/invalid RESEND_API_KEY, unverified sender domain, Resend API errors) were only logged and looked exactly like success. On production, the stale Render deployment (blocked by the iter109 PyPI issue) and/or missing env vars made this silent.
- **Fix** (`routers/b2b_waitlist.py` only): capture both `send_email` return bools into `email_status`; unconditionally persist `email_notifications {admin, applicant, error}` + `email_notifications_at` on the waitlist document; log an explicit error whenever admin delivery fails. The admin waitlist API (`GET /api/admin/b2b-waitlist`) returns full docs, so the status is visible in the admin panel data automatically.
- **Verified**: positive path with live RESEND_API_KEY records `admin: true`; Resend sandbox rejection of @example.com recipients now records `applicant: false` + error instead of vanishing; all GST anti-spoofing regressions (not-found, legal-name/state/pincode mismatch) still 400; GSTIN login regression green.
- **USER ACTION REQUIRED on production**: push to GitHub ("Save to Github"), redeploy Render, and confirm Render env has `RESEND_API_KEY`, `ADMIN_EMAIL=contact.us@centraders.com`, `SENDER_EMAIL` (domain verified in Resend). After redeploy, do one test signup and check the waitlist doc: `email_notifications.admin` must be `true` — if `false`, the `error` field now says exactly what's wrong.
