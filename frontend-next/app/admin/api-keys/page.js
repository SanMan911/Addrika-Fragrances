  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6" data-testid="admin-api-keys-page">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-[#1e3a52] dark:text-slate-200 uppercase tracking-wider mb-3">Generate a new key</h2>
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            placeholder="Key name — e.g. Field Sales Manager"
            className="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-[#1e3a52] dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:border-[#D4AF37] outline-none"
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 dark:text-slate-500 text-sm" data-testid="api-keys-empty">No API keys yet. Create one above.</div>
        ) : (
              ...
              <div key={k.id} className="px-5 py-4 flex items-center justify-between gap-4 flex-wrap" data-testid={`api-key-row-${k.id}`}>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-[#1e3a52] dark:text-slate-100">{k.name}</span>
                    {k.is_active ? (
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400 mt-1 flex flex-wrap gap-x-4 gap-y-1">
                    <span>Prefix: <code className="bg-gray-100 dark:bg-slate-700 dark:text-slate-200 px-1 rounded">{k.key_prefix}</code></span>
